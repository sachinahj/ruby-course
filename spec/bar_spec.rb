...
describe Bar do
  ...
  it "only returns a discount when it's happy hour" do
    ...
    expect(@bar.happy_discount).to_not eq 0
  ...
  it "constrains its happy hour discount to between zero and one" do
    ...
    Time.stub(:now).and_return(Time.parse('June 4 2014 3:45 pm'))
...
# # # # # # # # # # # # # # # # # # # # # #
  ...
  describe 'extra tests', :pending => false do
    ...
    it "has full discount on mondays and wednesdays" do
      time = Time.parse("June 4 2014 3:00 pm") # your start
      # expect(Time).to receive(:now).and_return(time)
      Time.stub(:now).and_return(time)
      expect(@bar.happy_discount).to eq(0.25)   
   end
   it "has half discount on days other than mondays and wednesdays" do
      time = Time.parse("June 3 2014 3:00 pm") # your start
      # expect(Time).to receive(:now).and_return(time)
      Time.stub(:now).and_return(time)
      expect(@bar.happy_discount).to eq(0.125)  
   end

   it "determines drink discount based on happy_hour? and item passed" do
    beer = Item.new("Beer", 4)
    margarita = Item.new("Margarita", 5)
    wine = Item.new("Wine", 6)
    vodka = Item.new("Vodka", 7, false)
    whiskey = Item.new("Whiskey", 8, false)
    @bar.menu_items.push(beer, margarita, wine, vodka, whiskey)
    p @bar.menu_items
    time = Time.parse("June 4 2014 3:00 pm")
    Time.stub(:now).and_return(time)
    expect(@bar.drink_discount("beer")).to eq(0.25) 
    expect(@bar.drink_discount("whiskey")).to eq(0) 
   end
  end
end
